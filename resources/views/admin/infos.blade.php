...
@section('content')
    <div class="container">
        <h3 class="text-center title_margin" style="margin: 24px 0">Інфо</h3>
            ...
            <table class="table admin_3_table_sm">
                ...
                @foreach ($infos as $info)
                    <tr data-id="{{$info->id}}">
                        ...
                        <td style="min-width: 150px;">{{$info->title_ua}}</td>
                        <td style="width: 160px;">
                            <a href="{{url("/admin/edit-info/".$info->id)}}" type="button" class="btn btn-primary admin_button_SaniaZaebalEdition">
                            ...
                            <button type="button" class="btn admin_button_SaniaZaebalEdition change-visible @if($info->hidden) btn-danger @else btn-success @endif" data-id="{{$info->id}}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            ...
                            <button type="button" class="btn trash btn-danger admin_button_SaniaZaebalEdition" data-id="{{$info->id}}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
    ...
    <script>
        ...
        const acceptDelete = () => {
            console.log(trashId);
            post('{{url('/admin/info-delete')}}', {id: trashId}, res => document.querySelector(`tr[data-id='${trashId}']`).remove());
        }

        const handleChangeVisibleClick = (e) => {
            const btn = btnFromEvent(e);
            const id = btn.dataset.id;
            post('{{url('/admin/info-change-visible')}}', {id}, res => {
                if (res.error) return;
                if(res.hidden){
        ...
        buildModal("danger", "Видалення категорії корисної інформації", "Ви впевнені, що хочете видалити цю категорію корисної інформації?", document.querySelector("#trash-modal"), acceptDelete);
    </script>
@stop
