...
@section('content')
    <div class="container">
        <h3 class="text-center title_margin" style="margin: 30px 0 30px">Категорії подій</h3>
        <div style="display: flex; justify-content: flex-end; margin: 20px 0 10px;">
            <a href="{{url("/admin/create-topic-event")}}" class="btn btn-primary">
                ...
                @foreach ($topics as $topic)
                    <tr data-id="{{$topic->id}}">
                        ...
                        <td style="width: 100px;">
                            <a href="{{url("/admin/edit-topic-event/".$topic->id)}}" type="button" class="btn btn-primary admin_button2">
    ...
    <script>
        ...
        const acceptDelete = () => {
            post('{{url('/admin/topic-event-delete')}}', {id: trashId}, res => document.querySelector(`tr[data-id='${trashId}']`).remove());
        }

        const handleChangeVisibleClick = (e) => {
            const btn = btnFromEvent(e);
            const id = btn.dataset.id;
            post('{{url('/admin/topic-event-change-visible')}}', {id}, res => {
                if (res.error) return;
                if(res.hidden){
